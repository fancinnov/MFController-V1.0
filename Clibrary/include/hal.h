...
extern MPU6887_Data mpu6887_data;
extern QMI8658_Data qmi8658_data;
...
void COMM5_Callback(void);//串口5中断回调函数
void COMM6_Callback(void);//串口6中断回调函数
void COMM7_Callback(void);//串口7中断回调函数
void COMM8_Callback(void);//串口8中断回调函数
...
void Led_rgb_red(bool status);
void Led_rgb_green(bool status);
void Led_rgb_blue(bool status);
...
extern uint8_t COMM_0, COMM_1, COMM_2, COMM_3, COMM_4, COMM_5;
/***************usb+串口配置****************
 * *************comm0:USB口***************
 * *************comm1:Serial串口1**********
 * *************comm2:Serial串口2**********
 ...
 * **************************************/
void config_comm(uint8_t comm0, uint8_t comm1, uint8_t comm2, uint8_t comm3, uint8_t comm4, uint8_t comm5);
...
uint16_t get_comm5_available(void);	//判断串口5是否有数据收到,有数据收到则返回接收到的byte数,没有数据收到返回0;	(注意：该函数只有在串口5是自定义模式 DEV_COMM 时才有效)
...
uint8_t get_comm5_data(void);	//读取串口5收到的数据,每调用一次可以读取1个字节;	(注意：该函数只有在串口5是自定义模式 DEV_COMM 时才有效)
...
void get_sub_data(void);
void send_sub_data(void);
//usb port
...
void set_s5_baudrate(uint32_t baudrate);//配置串口5波特率，默认初始波特率为115200
/**
...
void s5_printf_dir(const char* s, ...);
/**
...
/**
  * @brief  采用中断方式从串口5发送数据
  * @param  buf: 待发送数据的数组起始地址
  * @param  size: 待发送数据的字节长度
  * @retval 串口发送状态
  */
HAL_StatusTypeDef s5_send_buf(uint8_t* buf, uint16_t size);
...
HAL_StatusTypeDef s5_send_buf_delayms(uint8_t* buf, uint16_t size, uint32_t timeout);
...
#define EVENTBIT_HEARTBEAT_COMM_5 (1<<5) //串口5
#define EVENTBIT_HEARTBEAT_COMM_6 (1<<6) //板载wifi口
extern uint8_t HeartBeatFlags;//判断EVENTBIT_HEARTBEAT_COMM_0~EVENTBIT_HEARTBEAT_COMM_6通道是否有心跳包接收
