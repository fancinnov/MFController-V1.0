...
static LowPassFilterVector2f _odom_vel_filter, _odom_pos_filter, _air_resistance_filter;
...
//接收
static uint32_t time_last_heartbeat[7]={0};
...
void send_mavlink_heartbeat_data(void){
	...
	if ((COMM_5&COMM_MASK)==MAV_COMM){
		mavlink_send_buffer(MAVLINK_COMM_5, &msg_heartbeat);
		if(gcs_channel==MAVLINK_COMM_5){
			mavlink_send_buffer(MAVLINK_COMM_5, &msg_battery_status);
		}
	}
	mavlink_send_buffer(MAVLINK_COMM_6, &msg_heartbeat);
	if(gcs_channel==MAVLINK_COMM_6){
		mavlink_send_buffer(MAVLINK_COMM_6, &msg_battery_status);
	}
}
...
void update_accel_gyro_data(void){
	imu_healthy=true;
	accel.x = mpu6887_data.accf.x;//m/s/s
	accel.y = mpu6887_data.accf.y;//m/s/s
	accel.z = mpu6887_data.accf.z;//m/s/s
	...
	if((is_equal(accel.length(),0.0f)) || isnan(accel.x) || isinf(accel.x) || isnan(accel.y) || isinf(accel.y) || isnan(accel.z) || isinf(accel.z)){
		...
		if((is_equal(accel.length(),0.0f)) || isnan(accel.x) || isinf(accel.x) || isnan(accel.y) || isinf(accel.y) || isnan(accel.z) || isinf(accel.z)){
			accel.x = qmi8658_data.accf.x;//m/s/s
			accel.y = qmi8658_data.accf.y;//m/s/s
			accel.z = qmi8658_data.accf.z;//m/s/s
			if((is_equal(accel.length(),0.0f)) || isnan(accel.x) || isinf(accel.x) || isnan(accel.y) || isinf(accel.y) || isnan(accel.z) || isinf(accel.z)){
				return;
			}
		}
...
//	usb_printf("ax:%f,ay:%f,az:%f\n",accel.x,accel.y,accel.z);
	gyro.x = mpu6887_data.gyrof.x;//rad/s
	gyro.y = mpu6887_data.gyrof.y;//rad/s
	gyro.z = mpu6887_data.gyrof.z;//rad/s

	if((is_equal(gyro.length(),0.0f)) || isnan(gyro.x) || isinf(gyro.x) || isnan(gyro.y) || isinf(gyro.y) || isnan(gyro.z) || isinf(gyro.z)){
		imu_healthy=false;
		gyro.x = adis16470_data.gyrof.x;//rad/s
		gyro.y = adis16470_data.gyrof.y;//rad/s
		gyro.z = adis16470_data.gyrof.z;//rad/s
		if((is_equal(gyro.length(),0.0f)) || isnan(gyro.x) || isinf(gyro.x) || isnan(gyro.y) || isinf(gyro.y) || isnan(gyro.z) || isinf(gyro.z)){
			gyro.x = qmi8658_data.gyrof.x;//rad/s
			gyro.y = qmi8658_data.gyrof.y;//rad/s
			gyro.z = qmi8658_data.gyrof.z;//rad/s
			if((is_equal(gyro.length(),0.0f)) || isnan(gyro.x) || isinf(gyro.x) || isnan(gyro.y) || isinf(gyro.y) || isnan(gyro.z) || isinf(gyro.z)){
				return;
			}
		}
...
void update_baro_alt(void){
	...
	float baro_alt=spl06_data1.baro_alt*100.0f;
...
float get_baro_temp(void){//℃
	return (spl06_data1.temp+spl06_data2.temp)/2;
}
...
void ekf_gnss_xy(void){
	if(!ahrs->is_initialed()||(!ahrs_healthy)||(!get_gps_state()&&!get_opticalflow)||offboard_enable){
		return;
	}
...
void Logger_Data_Callback(void){
	...
	sd_log_write("%8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8d ",//LOG_SENSOR
			get_mag_filt().x, get_mag_filt().y, get_mag_filt().z, spl06_data1.baro_alt, get_power_volt(), get_power_current(), gps_position->satellites_used);
	osDelay(1);
	...
	osDelay(1);
	sd_log_write("%8.3f %8.3f %8.3f %8.3f %8.3f %8.3f ",//LOG_SENSOR
			qmi8658_data.accf.x, qmi8658_data.accf.y, qmi8658_data.accf.z, qmi8658_data.gyrof.x, qmi8658_data.gyrof.y, qmi8658_data.gyrof.z);
	osDelay(1);
...
//	usb_printf("%f\n",motors->get_throttle());
//	dataflash->set_param_float(param->angle_max.num, DEFAULT_ANGLE_MAX);
//	dataflash->get_param_float(param->angle_max.num, param->angle_max.value);
...
//	usb_printf("baro:%f,",spl06_data1.baro_alt);
//	usb_printf("temp:%f\n",spl06_data1.temp);
//	usb_printf("alt:%f\n",get_alt_target());
...
//	usb_printf("ax:%f,ay:%f,az:%f\n",adis16470_data.accf.x,adis16470_data.accf.y,adis16470_data.accf.z);
//	usb_printf("%f,%f,%f\n",qmi8658_data.gyrof.x, qmi8658_data.gyrof.y, qmi8658_data.gyrof.z);
//	usb_printf("%f,%f,%f\n",attitude->get_att_target_euler_d().x, attitude->get_att_target_euler_d().y, attitude->get_att_target_euler_d().z);
...
//	s2_printf("x:%f,y:%f\n", x_target, y_target);
//	usb_printf("pos_z:%f|%f|%f|%f\n",spl06_data1.baro_alt,get_pos_z(),get_vel_z(), baro_alt_filt);
